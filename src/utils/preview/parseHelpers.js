...
const parseQuickstartDir = (file) => {
  ...
  if (file.type === 'yaml') {
    ...
    // if documentation itterate through the array of documentation objects to trim new lines
    let trimmedDocumentation;

    if (loadYaml.documentation) {
      trimmedDocumentation = loadYaml.documentation.map((doc) => {
...
export const parseQuickstartFiles = async (rawFileContent) => {
